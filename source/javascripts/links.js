...
(function() {
  ...
  function createBookmarklet(){
    ...
    // build list of links
    var linksOnPage = $("a[href]");
    var links = "<ul>";
    var texts = [];
    linksOnPage.each(function(){
      var $this = $(this);
      var link = $this.html()
      if (link != "" && link === $this.text() && !texts[link]) {
        texts[link] = true;
        links+="<li><a target=_blank href='"+this.href+"'>"+$(this).html()+"</a></li>";
      }
    });
    // display html
    $("body").append(htmlTemplate(links+"</ul>"));
  }
  ...
  .wait(function(){
    ...
    if ($el.length === 0) {
      ...
      $("#huge-bookmarklet-template-lightbox").show();
    }
  });
})();